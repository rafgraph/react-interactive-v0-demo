...
class ShowInfo1 extends React.Component {
  constructor() {
    super();
    this.state = {
      showInfo: false,
...
// shown on: toggle on focus (include touchActive)
class ShowInfo2 extends React.Component {
  constructor() {
    super();
    this.state = {
      showInfo: false,
  ...
  handleStateChange = ({ nextState }) => {
    if (nextState.focus || nextState.iState === 'touchActive') {
      this.setState({ showInfo: true });
    } else {
      this.setState({ showInfo: false });
    }
      ...
      <div>
        <div style={s.infoBox}>
          {this.state.showInfo ?
            <span style={s.infoText}>Some info to show about something</span>
            :
            <span style={s.subTitle}>{s.code('focus')} (toggles) and {s.code('touchActive')}</span>
          }
        </div>
        <svg
          xmlns="http://www.w3.org/2000/svg"
